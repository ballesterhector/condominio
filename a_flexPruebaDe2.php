			...
			<div id="main">
						...
						<form id="formulariodetalle" class="form-horizontal" onsubmit="return agregarDetalle();">
							<div class="subladotop">
								...
								<label for="" class="">Concepto
									<input type="text" class="form-control " name="concep" id="concep"  style="width:360px" required="require">
								</label>
								...
								<label for="" class="">Unidades
									<select class="form-control" name="unidad" id="unidad" required="require" autocomplete="off" onclick="diferencia()">
										<option></option>
										<?php
											$obj = new conectarDB();
											$data = $obj->consultar("CALL unidadesSelect()");
											foreach($data as $key){
												echo "<option value='".$key['unidad']."'>".$key['unidad']."</option>";
											}
										?>
									</select></label>
								<label>
						...
						<table class="table table-condensed table-bordered table-responsive display compact" id="dataTables">
							...
							<tfoot>
								<tr style="font-size:10px">
									<th colspan="3" style="text-align:right">Ejecutado:</th>
									<th class="dt-right"><?php echo $totunida ?></th>
									<th class="dt-right"><?php echo number_format($gasto,2) ?></th>
								</tr>
								<tr style="font-size:10px">
									<th colspan="4" style="text-align:right">Por ejecutar:</th>
							...
							<tbody style="font-size:10px">
								<?php
									...
									if ($dat=='null') {
										...
										foreach ($data as $filas) { ?>
												<tr>
												<td><?php echo $filas['numegasto'] ?></td>
				...
				<aside>
					...
					<label for="" class="">Código
						<input type="text" class="form-control " name="numer" id="numer" value="<?php echo $numero ?>" readonly style="width:80px;text-align:center">
					</label>
					<label for="" class="">Fecha
						<input type="date" class="form-control " name="fech" id="fech" value="<?php echo $fecha ?>" disabled="true">
					</label>
					<label for="" style="margin-left:5px;"> Residencia
						<select class="form-control" name="conjunto" id="conjunto" value="<?php echo $resid ?>" disabled="true">
							<?php
								$obj = new conectarDB();
					...
					<label for="" style="margin-left:5px;"> Proveedor
						<select class="form-control" name="provee" id="provee" value="<?php echo $numprovee ?>" disabled="true">
							<?php
								$obj = new conectarDB();
					...
					<label for="" class="">Factura
						<input type="text" class="form-control lete" name="factu" id="factu" value="<?php echo $factura ?>" style="width:120px" disabled="true">
					</label>
					<label for="" class="">Periodo
						<input type="text" class="form-control lete" name="period" id="period" value="<?php echo $periodo ?>" style="width:90px;text-align:center" disabled="true">
					</label>
					<label for="" class="">Total Gasto
						<input type="text" class="form-control lete" name="totg" id="totg" value="<?php echo number_format($totfac,2) ?>" style="width:100px;text-align:center" disabled="true">
					</label>
